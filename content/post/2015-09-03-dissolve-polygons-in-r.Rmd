...
- `sf` functions do not drop the data frame, which many of the `rgdal` functions did.
...
There are now two ways to effectively dissolve the internal (region) boundaries.
One can be used to preserve relevant data (i.e. numerical data), while the other simply drops all the data except the geometry column.
I'll do one example of each.

If you want to preserve the data frame I presume you have some data that's meaningful when aggregated, such as area or other numerical indicator that can be summed.
My example data doesn't have this so lets create it:
