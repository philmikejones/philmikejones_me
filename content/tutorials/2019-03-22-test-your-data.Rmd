...
The minimum number of units should be `0.0`, while the total maximum should be the sum of the individual maximum number of units.

```{r test-range}
test_that("Calculated units are in plausible range", {
  expect_gte(min(bcs70$total_units), 0.0)
  expect_lte(max(bcs70_total_units), )
})
```
